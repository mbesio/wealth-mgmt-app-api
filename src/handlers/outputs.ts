...
export const getTimeseries = async (req, res) => {
  ...
  const inputLiabilitiesEURUSDByDate = combineAmountsByDate(inputLiabilitiesEURUSD)

  // combine asset and input dates
  const dates = combineDates(inputAssetsEURUSDByDate, inputLiabilitiesEURUSDByDate)
  ...
  const timeseries = dates.map(item => {
    ...
    const liabilites = find(inputLiabilitiesEURUSDByDate, {date: item.date}) || 0
    return {
      date: item.date,
      assets: assets,
