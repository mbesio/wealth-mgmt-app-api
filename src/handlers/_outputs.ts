...
import { getInputLiabilitesTimeSeries, getLiabilitesAsOfDate } from '../utils/liabilities'
...
export const getSummaryMetrics = async (req, res) => {
  ...
  const liabilitiesWithCurrencies = getLiabilitesAsOfDate(currentDate)

  res.json({
    data: {
      assets: inputAssetsWithCurrencies,
      liabilites: liabilitiesWithCurrencies
    }
  })
}